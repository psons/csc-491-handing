...
//  Class defenitions above to copy to project.   Demo Code below.

func demoEffortDomain() {
    print("")
    print("==== EffortDomain - Codable")

    let effortDomainJsonString =
    """
    {
...
func demoGoal() {
    ...
    """
    let goalData = goalJsonString.data(using: .utf8)!
...
func demoObjective() {
    print("")
    print("==== Objective - Codable")
    let objectiveJsonString =
    """
                {
                    "maxTasks": 3,
                    "name": "new task story with a long name that wraps past first line in a separator",
    ...
    """
    let objectiveData = objectiveJsonString.data(using: .utf8)!
    let objective: Objective = try! JSONDecoder().decode(Objective.self, from: objectiveData)
    print(objective)
...
func demoTask() {
    ...
    """
    let taskData = taskJsonString.data(using: .utf8)!
    let task: EGTask = try! JSONDecoder().decode(EGTask.self, from: taskData)
    print(task)
    
    let t1 = EGTask(status: .todo, name: "Seize political control with Swift", detail: "Write the killer app that everybody worships")
    print(t1)
...
func demoStatusVal() {
    ...
    //https://stackoverflow.com/questions/44580719/how-do-i-make-an-enum-decodable-in-swift
    struct ThingWithStatus: Codable, CustomStringConvertible {
        let status: StatusVal
        var description: String {
            return "{ThingWithStatus} |status: \(status)|"
        }
    }

    print("")
    print("==== StatusVal - Codable")
    let thingWithStatusJsonString =
    """
    {
    "status": "in_progress"
    }
    """
    let thingWithStatusData = thingWithStatusJsonString.data(using: .utf8)!
    let thingWithStatus: ThingWithStatus = try! JSONDecoder().decode(ThingWithStatus.self, from: thingWithStatusData)
    print("object as Str: ", thingWithStatus)

    let encoder = JSONEncoder()
    encoder.outputFormatting = .prettyPrinted

    let data = try! encoder.encode(thingWithStatus)
    let thingWithStatus_NewJsonString = String(data: data, encoding: .utf8)!
    print("re-encoded as JSON: \n", thingWithStatus_NewJsonString)
}
